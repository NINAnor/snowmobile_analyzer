...
a <- ggplot(data=df_cm, aes(x=Filtering, y=variable, fill=Type, alpha=Proportion)) +
  geom_tile() +
  geom_text(aes(label = values, vjust = .5, fontface  = "bold", alpha = 1), show.legend = FALSE) +
...
b <- ggplot(data=df_metrics, aes(x=Filtering, y=variable, fill=values)) +
  ...
  geom_text(aes(label = values), vjust = .5, fontface  = "bold", alpha = 1) +
  theme_classic() +
  labs(
...
a / b + plot_annotation(tag_level = "A") & theme(
